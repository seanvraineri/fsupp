...
async function processGeneticFileWithAI(arrayBuf: ArrayBuffer, format: string, supabase: any, fileRow: any) {
  ...
  // Extract highlights
  console.log('FAST PROCESSING WITH AI - Extracting highlights...');
  const highlights = extractHighlights(snpData);
  console.log('FAST PROCESSING WITH AI - Highlights extracted:', highlights);
  
  // Save to database immediately
  console.log('FAST PROCESSING WITH AI - About to save genetic data...');
  try {
    await saveGeneticData(supabase, fileRow, snpData, highlights);
    console.log('FAST PROCESSING WITH AI - Genetic data save completed successfully');
  } catch (saveError) {
    console.error('FAST PROCESSING WITH AI - Error saving genetic data:', saveError);
...
async function saveGeneticData(supabase: any, fileRow: any, snpData: Record<string, string>, highlights: Record<string, any>) {
  console.log(`FAST PROCESSING WITH AI - Testing database connection...`);
  
  // Test database connection first
  try {
    const { data: testData, error: testError } = await supabase.from("genetic_markers").select("id").limit(1);
    if (testError) {
      console.error("FAST PROCESSING WITH AI - Database connection test failed:", testError);
      throw testError;
    }
    console.log(`FAST PROCESSING WITH AI - Database connection test passed`);
  } catch (connError) {
    console.error("FAST PROCESSING WITH AI - Database connection error:", connError);
    throw connError;
  }
  ...
  console.log(`FAST PROCESSING WITH AI - Saving genetic data for file_id: ${fileRow.id}`);
  console.log(`FAST PROCESSING WITH AI - Row data:`, JSON.stringify(row, null, 2));

  try {
    // Try delete first, then insert
    const { error: deleteError } = await supabase.from("genetic_markers").delete().eq("file_id", fileRow.id);
    if (deleteError) {
      console.log("FAST PROCESSING WITH AI - Delete error (expected if no existing record):", deleteError);
    }
    
    const { data, error } = await supabase.from("genetic_markers").insert(row);
    if (error) {
      console.error("FAST PROCESSING WITH AI - Database error:", error);
      throw error;
    }
    console.log(`FAST PROCESSING WITH AI - Successfully saved genetic data:`, data);
  } catch (dbError) {
    console.error("FAST PROCESSING WITH AI - Failed to save genetic data:", dbError);
    throw dbError;
  }
}
