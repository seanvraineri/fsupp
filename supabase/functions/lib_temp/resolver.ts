...
async function fetchJsonWithProxy(url:string){
...
async function searchDSLD(query: string): Promise<ResolvedProduct | null> {
  ...
  const r = await fetchJsonWithProxy(url);
  if (!r.ok) return null;
  let j: any;
  try {
    j = await r.json();
  } catch (_) {
    // Non-JSON (Cloudflare or other)
    return null;
  }
...
export async function resolveProduct(p: ResolverPayload): Promise<ResolvedProduct> {
  if (p.text) {
    ...
    if (!resolved) {
      resolved = await searchSerpApi(p.text);
    }
    if (resolved) return resolved;
    // ultimate fallback dummy product
